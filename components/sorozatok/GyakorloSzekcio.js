...
const jeles = (c, valtozo = "") => `${c >= 0 ? "+" : "-"}${Math.abs(c) === 1 && valtozo ? "" : Math.abs(c)}${valtozo}`;
...
function polinomFeladat() {
  ...
  const A = a / d;
  const kiertekel = (egy, n) => egy.reduce((sum, c, i) => sum + c * Math.pow(n, egy.length - 1 - i), 0);
  const a5 = kiertekel(szamlalo, 5) / kiertekel(nevezo, 5);

  return {
    szoveg: (
      <p>
        Számítsd ki a határértéket, és add meg az <M>{"a_5"}</M> tagot is (3 tizedesre):
        <MB>{`\\lim_{n\\to\\infty}\\frac{${polinom(szamlalo)}}{${polinom(nevezo)}}`}</MB>
      </p>
    ),
    sugo: (
      <p>
        Ossz el minden tagot a nevező legmagasabb fokú tagjával, itt <M>{`n^{${fok}}`}</M>-nal. Utána minden{" "}
        <M>{"1/n^k"}</M> alakú tag nullához tart, és a maradék leolvasható. Azonos fokszámnál a válasz mindig a{" "}
        <strong>főegyütthatók hányadosa</strong>.
      </p>
    ),
    mezok: [
    ...
    megoldas: (
      <>
        ...
        <p>
          A számláló és a nevező külön-külön konvergens, és a nevező határértéke <M>{`${d} \\ne 0`}</M> — csak ezért szabad a
          műveleti tételt alkalmazni.
        </p>
...
function gyokosFeladat() {
  ...
  if (tipus === "egy") {
    ...
    return {
      szoveg: (
        <p>
          Számítsd ki a határértéket (3 tizedesre), és add meg a konjugálttal való bővítés után a{" "}
          <strong>számlálóban</strong> maradó kifejezés <M>{"n"}</M>-es együtthatóját:
          <MB>{`\\lim_{n\\to\\infty}\\left(\\sqrt{${polinom([1, a, b])}} - n\\right)`}</MB>
        </p>
      ),
      sugo: (
        <p>
          <M>{"\\infty-\\infty"}</M> alak: bővíts a konjugálttal, vagyis szorozz és ossz a{" "}
          <M>{`\\sqrt{${polinom([1, a, b])}} + n`}</M> kifejezéssel. A számlálóban az{" "}
          <M>{"(x-y)(x+y) = x^2-y^2"}</M> azonosság miatt eltűnnek a gyökök.
        </p>
      ),
      mezok: [
        { id: "A", cimke: "A határérték", helyes: A, tizedes: 3 },
        { id: "c", cimke: "n együtthatója a számlálóban", helyes: a, tizedes: 0, tures: 0.01 },
      ],
      megoldas: (
        <>
          <MB>{`\\sqrt{${polinom([1, a, b])}} - n = \\frac{\\left(${polinom([1, a, b])}\\right) - n^2}{\\sqrt{${polinom([1, a, b])}} + n} = \\frac{${polinom([a, b])}}{\\sqrt{${polinom([1, a, b])}} + n}`}</MB>
          <p>
            Most osztunk <M>{"n"}</M>-nel; a <strong>gyökjel alatt ez n²-tel való osztást jelent</strong>:
          </p>
  ...
  return {
    ...
    sugo: (
      <p>
        Két gyök különbsége <M>{"\\infty-\\infty"}</M> alak. Bővíts az összegükkel: a számlálóban{" "}
        <M>{`(n^2+${a}n) - (n^2+${b}n)`}</M> marad, ami már csak elsőfokú.
      </p>
    ),
    mezok: [
      { id: "A", cimke: "A határérték", helyes: A, tizedes: 3 },
      { id: "c", cimke: "n együtthatója a számlálóban", helyes: a - b, tizedes: 0, tures: 0.01 },
    ],
    megoldas: (
      <>
        ...
        <p>
          ...
          <M>{szK(Math.sqrt(1e6 + a * 1000) - Math.sqrt(1e6 + b * 1000), 4)}</M> — valóban <M>{szK(A, 3)}</M> közelében.
        </p>
      </>
    ),
  };
...
function kuszobFeladat() {
  ...
  return {
    szoveg: (
      <p>
        Az <M>{`a_n = \\frac{${polinom([a, b])}}{${polinom([c, d])}}`}</M> sorozat konvergens. Add meg az{" "}
        <M>{"A"}</M> határértéket (3 tizedesre), és azt a <strong>legkisebb</strong> <M>{"N"}</M> egész számot, amelyre{" "}
        <strong>minden</strong> <M>{"n > N"}</M> esetén <M>{`\\left|a_n - A\\right| < ${szK(eps, 2)}`}</M>.
      </p>
    ),
    sugo: (
      <p>
        Először a határérték (azonos fokszám). Utána közös nevezőre hozva{" "}
        <M>{`\\left|a_n - A\\right| = \\frac{${kul}}{${c}\\left(${polinom([c, d])}\\right)}`}</M>, és ezt kell{" "}
        <M>{szK(eps, 2)}</M> alá szorítani. A kapott korlát egész részéig „rossz” az index.
      </p>
    ),
    mezok: [
      { id: "A", cimke: "A határérték", helyes: A, tizedes: 3 },
    ...
    megoldas: (
      <>
        ...
        <p>
          A legkisebb ilyen egész küszöb tehát <M>{`N = ${N}`}</M>: az <M>{`${N + 1}`}</M>. tagtól kezdve minden tag{" "}
          <M>{szK(eps, 2)}</M>-nál közelebb van <M>{szK(A, 4)}</M>-hoz. Ellenőrzés:{" "}
          <M>{`\\left|a_{${N + 1}} - A\\right| = ${szK(Math.abs((a * (N + 1) + b) / (c * (N + 1) + d) - A), 5)}`}</M>, míg{" "}
...
function eTipusFeladat() {
    ...
    return {
      szoveg: (
        <p>
          ...
          <MB>{`\\lim_{n\\to\\infty}\\left(1 ${jeles(a, "")}\\cdot\\frac{1}{n}\\right)^{${b === 1 ? "" : b}n}`}</MB>
        </p>
      ),
      sugo: (
        <p>
          Alakítsd az alapot <M>{"1 + 1/b_n"}</M> formára: itt a „kicsi” rész <M>{`\\frac{${a}}{n}`}</M>, a reciproka{" "}
          <M>{`\\frac{n}{${a}}`}</M>. A kitevőt írd fel úgy, hogy ez a reciprok szorzóként megjelenjen.
        </p>
      ),
      mezok: [
      ...
      megoldas: (
        <>
          ...
          <MB>{`e^{${k}} = ${szK(A, 4)}`}</MB>
          <p>
            A szögletes zárójelben lévő rész <M>{"e"}</M>-hez tart, a külső kitevő pedig{" "}
            <M>{`\\frac{${b}n}{n/${a}} = ${k}`}</M>. A kitevőt soha ne hagyd el!
          </p>
        </>
      ),
    };
  ...
  return {
    ...
    sugo: (
      <p>
        Polinomosztással <M>{`\\frac{${polinom([1, a])}}{${polinom([1, b])}} = 1 + \\frac{${k}}{${polinom([1, b])}}`}</M>. A
        „kicsi” rész reciproka <M>{`\\frac{${polinom([1, b])}}{${k}}`}</M> — ezt kell a kitevőbe beépíteni.
      </p>
    ),
    mezok: [
      { id: "k", cimke: "e kitevője", helyes: k, tizedes: 0, tures: 0.01 },
    ...
    megoldas: (
      <>
        ...
        <p>
          A külső kitevő <M>{`\\frac{${k}n}{${polinom([1, b])}} \\to ${k}`}</M>, tehát a határérték{" "}
          <M>{`e^{${k}} = ${szK(A, 4)}`}</M>.
        </p>
      </>
    ),
...
function gyokNagysagrendFeladat() {
  ...
  if (tipus === "osszeg") {
    ...
    return {
      ...
      sugo: (
        <p>
          Rendőrelv: a nagyobbik hatvány már önmagában alsó becslés, a duplája felső becslés. A{" "}
          <M>{"\\sqrt[n]{2} \\to 1"}</M> miatt a két szélső sorozat ugyanoda tart.
        </p>
      ),
      mezok: [
        { id: "alap", cimke: "A domináns tag alapja", helyes: A, tizedes: 0, tures: 0.01 },
        { id: "A", cimke: "A határérték", helyes: A, tizedes: 3 },
      ],
      megoldas: (
        <>
          ...
          <p>
            Mivel <M>{"\\sqrt[n]{2} \\to 1"}</M>, mindkét szélső sorozat <M>{`${A}`}</M>-hoz tart, tehát a rendőrelv szerint a
            határérték <M>{`${A}`}</M>. Általánosan:{" "}
  ...
  if (tipus === "polinom") {
    ...
    return {
      ...
      sugo: (
        <p>
          A nagyságrendi sorrend szerint <M>{`n^{${k}} \\ll ${a}^n`}</M>, tehát nagy <M>{"n"}</M>-re{" "}
          <M>{`n^{${k}} \\le ${a}^n`}</M>. Ezzel a gyök alatti kifejezés két oldalról becsülhető.
        </p>
      ),
      mezok: [
        { id: "alap", cimke: "A domináns tag alapja", helyes: a, tizedes: 0, tures: 0.01 },
      ...
      megoldas: (
        <>
          ...
          <p>
            Az exponenciális erősebb minden hatványnál, ezért a polinomiális tag a gyök alatt „eltűnik”. A határérték{" "}
            <M>{`${a}`}</M>.
          </p>
        </>
      ),
  ...
  return {
    ...
    sugo: (
      <p>
        ...
        <M>{`\\left(\\frac{${a}}{${b}}\\right)^n \\to 0`}</M> (mert a hányados abszolút értéke 1-nél kisebb) és{" "}
        <M>{`\\frac{${s}}{${b}^n} \\to 0`}</M>.
      </p>
    ),
    mezok: [
      { id: "alap", cimke: "A domináns tag alapja", helyes: b, tizedes: 0, tures: 0.01 },
      { id: "A", cimke: "A határérték", helyes: A, tizedes: 3 },
    ],
    megoldas: (
      <>
        ...
        <p>
          A <M>{`\\left(\\frac{${a}}{${b}}\\right)^n`}</M> mértani sorozat hányadosa{" "}
          <M>{szK(a / b, 3)}</M>, abszolút értékben 1-nél kisebb, tehát nullához tart.
        </p>
      </>
    ),
  };
  ...
  return (
    <div>
      ...
      <GyakorloDoboz
        ...
        leiras={
          <>
            <M>{"1^{\\infty}"}</M> alak: hozd az alapot <M>{"1 + \\text{kicsi}"}</M> formára, és igazítsd a kitevőt. A külső
