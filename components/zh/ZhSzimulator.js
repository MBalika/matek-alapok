...
import { ZH_MODULOK as MODULOK } from "@/lib/zhFeladatok";

const KULCS = "matek-zh-elozmenyek";
...
function ujFeladatsor(valasztott) {
  const DB = 4;
...
export default function ZhSzimulator() {
  ...
  const indit = () => {
    setFeladatok(ujFeladatsor(valasztott));
    setValaszok({});
    setNyitott({});
    setHatra(perc * 60);
    setFazis("fut");
      ...
      <div className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
        <div className="rounded-2xl border border-[color:var(--keret)] bg-white p-5 sm:p-6">
          ...
          <ul className="mt-3 space-y-2 text-[14.5px] leading-relaxed text-petrol-700">
            <li>
              Négy véletlen feladat a kiválasztott modulokból — ugyanazokból a
              típusokból, mint a gyakorló dobozok, de{" "}
