 ...
 * sqrt, ln, lg, exp, sin, cos, tg, ctg, arcsin, arccos, arctg, sh, ch, th, abs, sgn,
 * fakt (faktoriális), pi, e. Függvényekhez: forditKifejezes(szoveg, ["x"]).
  ...
  while (i < s.length) {
    ...
    if (c >= "0" && c <= "9") {
      ...
      if (!Number.isFinite(szam))
        throw new Error(`Hibás szám: „${s.slice(i, j)}”`);
    ...
    if (/[a-zA-Z]/.test(c)) {
      ...
      if (valtozok.includes(nev))
        tokenek.push({ tipus: "valtozo", ertek: nev });
      else if (nev in ALLANDOK)
        tokenek.push({ tipus: "szam", ertek: ALLANDOK[nev] });
      else if (FUGGVENYEK.includes(nev))
        tokenek.push({ tipus: "fuggveny", ertek: nev });
...
function elemez(tokenek) {
  ...
  function tag() {
    ...
    if (t.tipus === "fuggveny") {
      vesz();
      if (!most() || most().tipus !== "(")
        throw new Error(`A(z) ${t.ertek} után zárójel kell.`);
      vesz();
      const belso = kifejezes(0);
      if (!most() || most().tipus !== ")")
        throw new Error("Hiányzó záró zárójel.");
      vesz();
  ...
  if (p !== tokenek.length)
    throw new Error("Fölösleges jelek a kifejezés végén.");
  return fa;
}
...
function forras(csomo) {
  switch (csomo.tipus) {
    ...
    case "muvelet":
      if (csomo.jel === "^")
        return `Math.pow(${forras(csomo.bal)},${forras(csomo.jobb)})`;
